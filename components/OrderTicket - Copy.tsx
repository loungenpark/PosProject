import React, { useEffect, useMemo, useRef } from 'react'; // Import useEffect and useRef
...
const OrderTicket: React.FC<OrderTicketProps> = ({ table, newItems, user }) => {
  const now = new Date();
  const hasPrinted = useRef(false); // Add a ref to track printing

  const total = useMemo(() => {
    return newItems.reduce((acc, item) => acc + (item.price * item.quantity), 0);
  }, [newItems]);

  // This useEffect will run after the component renders and trigger printing once.
  useEffect(() => {

...
import { EditIcon, TrashIcon, PlusIcon, CloseIcon, ChartBarIcon, MenuIcon, TableIcon, PercentIcon, UserGroupIcon, BoxIcon, PrinterIcon, UploadIcon } from './common/Icons';
                                            ...
                                            <tbody className="divide-y divide-accent/50">
                                                {sale.order.items.map(item => (
                                                    <tr key={item.id} className="text-text-secondary">
                                                        ...
                                                        <td className="text-right py-2">{formatCurrency(typeof item.price === 'string' ? parseFloat(item.price) : item.price)}</td>
...
const MenuManagement: React.FC = () => {
    // --- NEW: MenuImporter Component ---
    const MenuImporter: React.FC = () => {
        const { importMenuItemsFromCSV } = usePos();
        ...
        const handleFileChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
            ...
            if (!importMenuItemsFromCSV) {
                alert("Funksioni i importit nuk është implementuar ende.");
                return;
            }
        ...
        return (
            <div className="bg-primary p-4 rounded-lg border border-dashed border-accent mt-6">
                <h4 className="text-md font-semibold mb-2">Importo Artikujt nga skedari CSV</h4>
                <p className="text-sm text-text-secondary mb-3">
                    Ngarko një skedar CSV. Skedari **DUHET** të ketë këto kolona: <strong>Name, Price, Category, Printer</strong>.
                </p>
                <input
                    type="file"
                    accept=".csv"
                ...
                <label htmlFor="csv-importer" className={`inline-flex items-center space-x-2 px-4 py-2 bg-green-600 text-white rounded-md cursor-pointer hover:bg-green-700 ${isImporting ? 'opacity-50 cursor-not-allowed' : ''}`}>
                    ...
                    <span>{isImporting ? 'Duke importuar...' : 'Zgjidh Skedarin CSV'}</span>
                </label>
            </div>
        );
    };
    ...
    // --- MODIFIED: Added check for categories before opening form ---
    const handleAddItem = () => {
        if (menuCategories.length === 0) {
            alert("Ju lutemi shtoni një menu (kategori) fillimisht para se të shtoni një artikull.");
            setActiveSubTab('menus'); // Switch to the menus tab to guide the user
            return;
        }
        setEditingItem(null);
                            ...
                            <tbody className="divide-y divide-accent">
                                {menuItems.map(item => (
                                    <tr key={item.id}>
                                        ...
                                        <td className="p-3">{formatCurrency(typeof item.price === 'string' ? parseFloat(item.price) : item.price)}</td>
                                        <td className="p-3">
                                            <div className="flex space-x-2">

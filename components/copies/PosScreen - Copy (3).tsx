...
const PosScreen: React.FC = () => {
  ...
  useEffect(() => {
    if (activeTable) {
        setCurrentOrderItems(activeTable.order?.items || []);
    } else {
  ...
  const addToOrder = (item: MenuItem) => {
    ...
    setCurrentOrderItems(prevItems => {
        ...
        if (existingItem) {
            ...
            return [...prevItems, { ...item, quantity: 1, addedBy: loggedInUser.username }];
        }
    });
  };
                            ...
                            <li key={group.user}>
                                ...
                                <ul className="space-y-3">
                                    {group.items.map(item => (
                                        <li key={`${item.id}-${group.user}`} className="flex items-center bg-primary p-2 rounded-md">

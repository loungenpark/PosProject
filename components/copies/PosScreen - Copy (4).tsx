...
const PosScreen: React.FC = () => {
  ...
  useEffect(() => {
    if (activeTable && activeTable.order) {
        // When loading existing items, map over them and add the 'ordered' status
        const itemsWithStatus = activeTable.order.items.map(item => ({
            ...item,
            status: 'ordered'
        }));
        setCurrentOrderItems(itemsWithStatus);
    } else {
        // If it's a new order, the list is empty
  ...
  const addToOrder = (item: MenuItem) => {
    ...
    setCurrentOrderItems(prevItems => {
        const existingItem = prevItems.find(
          orderItem => orderItem.id === item.id && orderItem.addedBy === loggedInUser.username
        );

        if (existingItem) {
            return prevItems.map(orderItem =>
                orderItem.id === item.id && orderItem.addedBy === loggedInUser.username
                    ? { ...orderItem, quantity: orderItem.quantity + 1 }
                    : orderItem
            );
        } else {
            return [...prevItems, { ...item, quantity: 1, addedBy: loggedInUser.username, status: 'new' }];
        }
    });
  };
  ...
  const updateQuantity = (itemId: number, addedByUser: string, change: number) => {
    ...
    setCurrentOrderItems(prevItems => 
        prevItems
            .map(item =>
                (item.id === itemId && item.addedBy === addedByUser)
  ...
  const handleCancelOrder = useCallback(() => {
    ...
    logout(); // --- MODIFIED: Added logout on cancel ---
  }, [logout]);
  ...
  const handleSaveOrder = useCallback(async () => {
    ...
    const newItemsForTicket = currentOrderItems.filter(currentItem => 
      !savedItems.has(`${currentItem.id}-${currentItem.addedBy}-${currentItem.quantity}`)
    );
    ...
    setActiveTableId(null);
    logout(); // --- MODIFIED: Added logout on save ---
  }, [activeTableId, currentOrderItems, activeTable, saveOrderForTable, calculateNewOrderState, logout]);
  ...
  const handleFinalizeSale = async (amountPaid: number) => {
    ...
    setActiveTableId(null);
    logout(); // --- MODIFIED: Added logout on finalize ---
  };
    ...
    <div className="h-screen w-screen flex flex-col bg-primary">
      <header className="flex-shrink-0 bg-secondary flex items-center justify-between p-4 shadow-md">
        <div className="flex items-center space-x-3">
            <button onClick={handleCancelOrder} className="p-2 rounded-full text-text-secondary hover:bg-accent hover:text-white transition-colors">
      ...
      <div className="flex flex-grow overflow-hidden">
             ...
             <div className="flex space-x-2 overflow-x-auto pb-2 mb-4 flex-shrink-0">
                {menuCategories.map(category => (
                    <button 
                        ...
                        className={`px-4 py-2 rounded-md text-base font-bold whitespace-nowrap transition-colors ${selectedCategory === category.name ? 'bg-highlight text-white' : 'bg-accent text-text-secondary hover:bg-highlight hover:text-white'}`}
                    >
                        {category.name}
                    </button>
                ))}
        ...
        <aside className="w-full md:w-1/3 lg:w-1/4 flex-shrink-0 bg-secondary flex flex-col p-4 shadow-inner">
             <div className="flex justify-between items-center mb-4 border-b border-accent pb-2">
                ...
                <button onClick={handleCancelOrder} className="p-1 text-text-secondary hover:text-text-main">
                        ...
                        {groupedItems.map(group => (
                            <li key={group.user}>
                                <h3 className="text-sm font-bold text-text-secondary uppercase tracking-wider mb-2">
                                    Shtuar nga: {group.user}
                                </h3>
                                <ul className="space-y-3">
                                    {group.items.map(item => (

// THIS IS THE FINAL REPLACEMENT BLOCK (AROUND LINE 398)
					<li key={`${item.id}-${group.user}`} className={`flex items-center p-2 rounded-md ${item.status === 'ordered' ? 'bg-accent' : 'bg-primary'}`}>
					    ...
					    {/* If the item status is 'new', render the control buttons. */}
					    {/* If the item status is 'ordered', render the total quantity instead. */}
					    {item.status === 'new' ? (
					        <div className="flex items-center space-x-2">
					            <button
					                onClick={() => updateQuantity(item.id, item.addedBy, -1)}
					                className="p-1 rounded-full bg-secondary hover:bg-highlight transition-colors"
					            >
					                <MinusIcon className="w-4 h-4 text-text-main"/>
					            </button>
					            ...
					            <button
					                ...
					                className="p-1 rounded-full bg-secondary hover:bg-highlight transition-colors"
					            >
					                <PlusIcon className="w-4 h-4 text-text-main"/>
					            </button>
            
					            <button
					                onClick={() => removeFromOrder(item.id, item.addedBy)}

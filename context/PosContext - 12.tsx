...
export const PosProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const loadDataFromDb = useCallback(async () => {
    ...
    // Note: Tax rate will be loaded from the server or local DB, not reset to 0 here.
  }, []);

  // --- ✅ CHANGE #1: fetchAndCacheData is now safer ---
  // It only fetches "static" data (menu, users, tax) and will NOT overwrite local sales/history.
  const fetchAndCacheData = useCallback(async () => {
    if (!isBackendConfigured) return;
    try {
        // We only fetch data where the server is the single source of truth.
        const { users, menuItems, menuCategories, taxRate } = await api.bootstrap();
        
        await db.clearStaticData(); // Ensure this only clears static data stores
        await Promise.all([
          db.bulkPut(users, 'users'), 
          db.bulkPut(menuItems, 'menuItems'), 
          db.bulkPut(menuCategories, 'menuCategories')
        ]);
        ...
        // IMPORTANT: We no longer fetch sales/history here. This prevents overwriting local data.
        setIsOnline(true);
    } catch (error) { 
  ...
  }, []); // Removed fetchAndCacheData from dependencies
  ...
  const addHistoryEntry = useCallback(async (tableId: number, details: string) => {
    ...
    // This function should also likely save to IndexedDB and sync queue
  }, [loggedInUser]);
  ...
  // --- ✅ CHANGE #2: The final, correct startup logic ---
  // It is now free of syntax errors and uses the safer logic.
  useEffect(() => {
    const bootstrap = async () => {
        setIsLoading(true);
        await db.initDB();

        // Safer Table Loading Logic
  ...
  }, [setTableCount, syncOfflineData, loadDataFromDb, fetchAndCacheData]); // Corrected dependencies

...
export const PosProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const saveOrderForTable = useCallback(async (tableId: number, updatedOrder: Order | null, newItems: OrderItem[]) => {
    ...
    if (newItems.length > 0 && table && loggedInUser) {
        // This function correctly uses a socket emit to print, which is why it works from the phone.
        socket.emit('print-order-ticket', { table, newItems, user: loggedInUser });
    }
  }, [tables, updateOrderForTable, loggedInUser]);

  // --- CHANGE 1 START ---
  // The original addSale function has been modified to use Socket.IO for printing.
  const addSale = useCallback(async (order: Order, tableId: number) => {
    ...
    // REMOVED: printSaleReceipt(newSale);
    // ADDED: A socket emit to tell the server to broadcast a print receipt command.
    socket.emit('print-sale-receipt', newSale);
  ...
  useEffect(() => {
    ...
    // --- CHANGE 2 START ---
    // A new handler function to call the actual print utility.
    ...
    // --- CHANGE 3 START ---
    // The new listener that waits for the server's command to print the receipt.
    ...
    return () => {
      ...
      // --- CHANGE 4 START ---
      // Cleanup for the new listener.
      socket.off('print-sale-receipt-from-server', handlePrintSaleReceipt);
      // --- CHANGE 4 END ---
    };
  }, []);

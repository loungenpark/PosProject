// --- FINAL VERSION with Order Printing Logic Added ---
...
const setTaxRate = useCallback(async (ratePercent: number) => {
    // 1. Calculate the new rate as a decimal (e.g., 20% -> 0.2)
    const newRate = Math.max(0, ratePercent) / 100;

    // 2. Update the local React state immediately for a responsive UI
    setTaxRateState(newRate);

    // 3. Add the change to the offline sync queue
    await db.addToSyncQueue({
        type: 'SET_TAX_RATE',
        payload: { rate: newRate },
    });

    // 4. Log a history entry for this action
    await addHistoryEntry(0, `Tax rate updated to ${ratePercent}%`); // Using tableId 0 for system events

    // 5. If currently online, trigger an immediate sync
    if (isOnline) {
        await syncOfflineData();
    }
}, [isOnline, addHistoryEntry, syncOfflineData]); // <--- Make sure dependencies are updated
// --- END: REPLACEMENT CODE ---
  ...
  const saveOrderForTable = useCallback(async (tableId: number, updatedOrder: Order | null, newItems: OrderItem[]) => {
    ...
    if (newItems.length > 0 && table && loggedInUser) {
        // --- ✅ START: MODIFIED PRINT LOGIC ✅ ---
        // Step 1: Tell this machine to print if it's a print station
        printOrderTicket(table, newItems, loggedInUser);
        // Step 2: Tell the SERVER to broadcast a print message to all OTHER print stations
  ...
  }, [loggedInUser, tables, menuItems, isOnline, updateOrderForTable, addHistoryEntry, updateMenuItem]);
  ...
  useEffect(() => {
    ...
    // --- ✅ START: ADDED PRINT LISTENER ✅ ---
    // This is the new listener for print jobs from other devices
  ...
  useEffect(() => {
    const bootstrap = async () => {
        ...
        if (isBackendConfigured && navigator.onLine) {
            await syncOfflineData();
            await fetchAndCacheData(); // just added this line.
        } else {

...
export const PosProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const setTableCount = useCallback((count: number) => {
    ...
    setTables(currentTables => {
        ...
        for(let i = 0; i < count; i++) {
            newTables.push(currentTables[i] || { id: i + 1, name: `Tavolina ${i + 1}`, order: null });
        }
        return newTables;
    });
  }, []);
  ...
  useEffect(() => {
    const bootstrap = async () => {
        ...
        if (isBackendConfigured && isOnline) {
  ...
  }, [isOnline, setTableCount, syncOfflineData, fetchAndCacheData, loadDataFromDb]);

...
export const PosProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const fetchAndCacheData = useCallback(async () => {
    ...
    try {
        ...
        setIsOnline(false);
        // await loadDataFromDb(); // This will be called by the online/offline event handler
    }
  }, []);
  ...
  // CORRECTED: This function is now "non-destructive" and will preserve existing orders.
  ...
  useEffect(() => {
    ...
    // CORRECTED: Removed setTableCount from dependency array to prevent re-initialization.
  }, [syncOfflineData, loadDataFromDb]);
  ...
  const addUser = async (user: Omit<User, 'id'>) => {
    if (isOnline && isBackendConfigured) {
        try {
            ...
            setIsOnline(false);
            await db.addToSyncQueue({ type: 'ADD_USER', payload: user });
            const tempUser: User = { ...user, id: Date.now() };
            setUsers(prev => [...prev, tempUser]);
            await db.put(tempUser, 'users');
        }
    } else {
        await db.addToSyncQueue({ type: 'ADD_USER', payload: user });
        const tempUser: User = { ...user, id: Date.now() };
  ...
  const addMenuItem = async (item: Omit<MenuItem, 'id'>) => {
    if (isOnline && isBackendConfigured) {
        try {
            ...
            setIsOnline(false);
            await db.addToSyncQueue({ type: 'ADD_MENU_ITEM', payload: item });
            const tempItem: MenuItem = { ...item, id: Date.now() };
            setMenuItems(prev => [...prev, tempItem]);
            await db.put(tempItem, 'menuItems');
        }
    } else {
        await db.addToSyncQueue({ type: 'ADD_MENU_ITEM', payload: item });
        const tempItem: MenuItem = { ...item, id: Date.now() };
  ...
  const addMenuCategory = async (name: string) => {
    ...
    if (isOnline && isBackendConfigured) {
        try {
            const newCategory = await api.addMenuCategory(name);
            setMenuCategories(prev => [...prev, newCategory].sort((a,b) => a.name.localeCompare(b.name)));
            await db.put(newCategory, 'menuCategories');
        } catch (e) {
            setIsOnline(false);
            await db.addToSyncQueue({ type: 'ADD_MENU_CATEGORY', payload: { name } });
            const tempCategory: MenuCategory = { id: Date.now(), name };
            setMenuCategories(prev => [...prev, tempCategory].sort((a,b) => a.name.localeCompare(b.name)));
            await db.put(tempCategory, 'menuCategories');
        }
    } else {
        await db.addToSyncQueue({ type: 'ADD_MENU_CATEGORY', payload: { name } });
        const tempCategory: MenuCategory = { id: Date.now(), name };

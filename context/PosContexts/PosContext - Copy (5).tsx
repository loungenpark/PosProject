...
// --- NEW: Import PapaParse for CSV reading ---
...
// --- MODIFIED: Added import function to the interface ---
interface PosContextState {
  ...
  importMenuItemsFromCSV: (file: File) => Promise<{ itemsAdded: number, categoriesAdded: number, itemsSkipped: number }>; // New function
...
export const PosProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const addMenuCategory = async (name: string) => {
    if (menuCategories.some(cat => cat.name.toLowerCase() === name.toLowerCase())) {
        // Silent fail if category already exists, important for importer.
    ...
    return new Promise((resolve, reject) => {
        Papa.parse(file, {
            ...
            complete: async (results) => {
                ...
                const existingCategories = new Set(menuCategories.map(c => c.name.toLowerCase()));
                const existingItems = new Set(menuItems.map(i => i.name.toLowerCase()));

                for (const row of results.data as any[]) {
                    const name = row.Name?.trim();
                    const price = parseFloat(row.Price);
                    const category = row.Category?.trim();
                    let printer = row.Printer?.trim().toUpperCase();
                    ...
                    if (existingItems.has(name.toLowerCase())) {
                        console.warn(`Skipping duplicate item: ${name}`);
                        itemsSkipped++;
                        continue;
                    }

                    if (printer !== Printer.BAR && printer !== Printer.KITCHEN) {
                ...
                for (const newCatName of Array.from(newCategories)) {
                    try {
  ...
  const value = {
    ...
    importMenuItemsFromCSV, // <-- Expose the new function
    ...
    tableButtonSizePercent, setTableButtonSizePercent, // <-- TYPO FIXED
    taxRate, setTaxRate,
    history, saveOrderForTable,
  };

  return <PosContext.Provider value={value}>{children}</PosContext.Provider>; // <-- TYPO FIXED
};

export const usePos = (): PosContextState => {
  const context = useContext(PosContext); // <-- TYPO FIXED

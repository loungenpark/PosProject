...
export const PosProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const addMenuCategory = async (name: string) => {
    // --- MODIFIED: Check against current state to prevent duplicate API calls ---
    ...
    return new Promise((resolve, reject) => {
        Papa.parse(file, {
            ...
            complete: async (results) => {
                ...
                // --- FIX: Get the most up-to-date data directly from the database ---
                ...
                // Add all new, unique categories first
                ...
                for (const row of results.data as any[]) {
                    ...
                    const newItem: Omit<MenuItem, 'id'> = {
                        ...
                        printer: (row.Printer.trim().toUpperCase() === 'BAR' ? Printer.BAR : Printer.KITCHEN),
                        trackStock: false,
                        stock: Infinity,
                        stockThreshold: 0,
                    };
                    ...
                    try {
                        ...
                        existingItemNames.add(name.toLowerCase()); // Prevent duplicates within the same file
                        itemsAdded++;
                    } catch (error) {

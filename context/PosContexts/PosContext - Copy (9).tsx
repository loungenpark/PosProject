...
export const PosProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    socket.on('connect', () => console.log('✅ Real-time connection to server established.'));
    socket.on('disconnect', () => console.log('🔌 Real-time connection to server lost.'));

    const handleOrderUpdateFromServer = (updatedTables: Table[]) => {
    ...
    socket.on('order-updated-from-server', handleOrderUpdateFromServer);
    ...
    socket.on('tax-rate-updated-from-server', (newTaxRate: number) => {
        console.log('Received real-time tax rate update from another device.');
        if (typeof newTaxRate === 'number' && isFinite(newTaxRate)) {
            setTaxRateState(newTaxRate);
        }
    });
    ...
    return () => {
      ...
      socket.off('order-updated-from-server', handleOrderUpdateFromServer);
      socket.off('sale-finalized-from-server');
      socket.off('print-order-ticket-from-server');
      socket.off('tax-rate-updated-from-server');
    };
  }, []);
  ...
  const fetchAndCacheData = useCallback(async () => {
    ...
    try {
        ...
        if (typeof taxRate === 'number' && isFinite(taxRate)) {
  ...
  // --- MODIFIED: The single correct version of setTableCount ---
  const setTableCount = useCallback((count: number) => {
    ...
    setTables(currentTables => {
        ...
        for (let i = 0; i < count; i++) {
            if (currentTables && currentTables[i]) {
                newTables.push(currentTables[i]);
            } else {
                newTables.push({ id: i + 1, name: `${i + 1}`, order: null });
            }
        }
  ...
  useEffect(() => {
    const bootstrap = async () => {
        ...
        if (savedTablesJSON) {
            ...
            if (loadedTables.length !== savedTablesCount) {
  ...
  const addHistoryEntry = useCallback(async (tableId: number, details: string) => {
    ...
    const newEntry: HistoryEntry = {
        id: `hist-${Date.now()}`, tableId, timestamp: new Date(), user: loggedInUser, details,
    };
  ...
  const setTaxRate = async (ratePercent: number) => {
    ...
    if (isOnline && isBackendConfigured) {
        try {
            await api.updateTaxRate(newRate);
            socket.emit('tax-rate-update', newRate);
        } catch (error) {
            console.error("Failed to update tax rate on server:", error);
            alert("Ruajtja e normës së tatimit dështoi. Duke u rikthyer.");
            setTaxRateState(oldRate);
        }
    } else {
  ...
  const addSale = async (order: Order, tableId: number) => {
    ...
    const newSale: Sale = {
        id: `sale-${Date.now()}`, date: new Date(), order, user: loggedInUser, tableId: table.id, tableName: table.name,
    };
  ...
  useEffect(() => {
    if (saleToPrint) {
      ...
      if (isThisDeviceThePrintStation) {
        ...
        setTimeout(() => { window.print(); setSaleToPrint(null); }, 100);
      } else {
  ...
  useEffect(() => {
    if (orderToPrint) {
      ...
      if (isThisDeviceThePrintStation) {
        ...
        setTimeout(() => { window.print(); setOrderToPrint(null); }, 100);
      } else {
        ...
        Papa.parse(file, {
            ...
            complete: async (results) => {
                ...
                for (const row of results.data as any[]) {
                    ...
                    if (printer !== 'BAR' && printer !== 'KITCHEN') printer = 'KITCHEN';
                    if(printer === 'SHANK') printer = 'BAR';

                    if (!existingCategoryNames.has(category.toLowerCase())) {
                        if(!newCategoriesToCreate.has(category)) newCategoriesToCreate.add(category);
                    }
                }
                ...
                for (const row of results.data as any[]) {
                    ...
                    if (!name || existingItemNames.has(name.toLowerCase()) || !category || !finalCategoryNames.has(category.toLowerCase())) continue; 

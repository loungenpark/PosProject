...
// --- ✅ FINAL CHANGE: The tax endpoint is now a robust "UPSERT" ---
app.post('/api/settings/tax', asyncHandler(async (req, res) => {
    const { rate } = req.body;
    if (typeof rate !== 'number' || rate < 0) {
        return res.status(400).json({ message: 'Invalid tax rate provided.' });
    }
    // This SQL command will UPDATE the row if it exists, or INSERT it if it does not.
    // This permanently fixes the bug where the setting could not be saved for the first time.
    // NOTE: This requires your `settings` table to have a UNIQUE constraint on the `key` column.
    const upsertQuery = `

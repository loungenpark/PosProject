...
io.on('connection', (socket) => {
  ...
  // --- CHANGE START: THIS IS THE ONLY ADDITION NEEDED ---
  // Listen for the 'print-sale-receipt' event from any client (e.g., the phone).
  // Then, broadcast an event to all *other* clients so the Print Station can print it.
  socket.on('print-sale-receipt', (saleData) => {
    socket.broadcast.emit('print-sale-receipt-from-server', saleData);
  });
  // --- CHANGE END ---

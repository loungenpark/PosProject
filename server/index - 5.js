...
// This is the list of addresses allowed to talk to your server.
const allowedOrigins = [
  'http://localhost:3000',      // Allows the app to work when you test it on the PC
  'http://192.168.1.10:3000' // Allows the app to work on your Phone and on the PC via network IP
];
...
// Use the new, specific CORS options for all Express API routes
...
const io = new Server(httpServer, {
  // Also apply the stricter CORS policy to the real-time server
  cors: {
...
// 1. Define the path to the 'dist' folder created by 'npm run build'
const distPath = path.join(projectRoot, 'dist');

// 2. Tell Express to serve all static files (JS, CSS, images) from the 'dist' folder
app.use(express.static(distPath));
// --- END PRODUCTION FRONT-END SERVING ---
...
io.on('connection', (socket) => {
  ...
  socket.on('print-order-ticket', (orderData) => {
    socket.broadcast.emit('print-order-ticket-from-server', orderData);
  });
...
// --- ALL YOUR API ROUTES GO HERE (UNCHANGED) ---
...
app.delete('/api/menu-items/:id', asyncHandler(async (req, res) => {
    ...
    if (result.rowCount === 0) {
        console.log(`Attempted to delete menu item ${id}, but it was not found.`);
    }
    res.json({ success: true });
}));
...
app.post('/api/menu-categories/reorder', asyncHandler(async (req, res) => {
    ...
    orderedIds.forEach((id, index) => {
        queryText += `WHEN ${parseInt(id, 10)} THEN ${index + 1} `;
    });
...
app.post('/api/menu-items/reorder-from-csv', upload.single('reorderFile'), asyncHandler(async (req, res) => {
    ...
    orderedNamesFromCsv.forEach(name => {
        if (dbItemsMap.has(name)) {
            orderedIds.push(dbItemsMap.get(name));
        } else {
            notFoundCount++;
        }
    });
    ...
    if (orderedIds.length === 0) {
        return res.status(400).json({ message: 'No matching items found.', reorderedCount: 0, notFoundCount });
    }

    let queryText = 'UPDATE menu_items SET display_order = CASE id ';
    orderedIds.forEach((id, index) => {
        queryText += `WHEN ${id} THEN ${index + 1} `;
    });
    queryText += 'END WHERE id = ANY($1::int[])';
...
app.post('/api/settings/tax', asyncHandler(async (req, res) => {
    ...
    await query("UPDATE settings SET value = $1 WHERE key = 'taxRate'", [rate.toString()]);
    res.json({ success: true, newRate: rate });
}));


// --- CORRECTED CATCH-ALL ROUTE ---
// It must come AFTER all your API routes
app.get('*', (req, res) => {
    // For any request that isn't an API route, send the main index.html file from the 'dist' folder.
    // This is crucial for single-page application routing to work.
